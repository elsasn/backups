...
uk_dashboard = {
    init: function () {
        ...
        // large graph
        uk_dashboard.metrics_charts();
        // video player
        uk_dashboard.video_player();
        // calendar
        uk_dashboard.clndr_calendar();
        // google maps
        uk_dashboard.maplace_maps();

        // run animations after page is fully loaded
        $window.on('load',function(){
            uk_dashboard.count_animated();
        });
    },
    // metrics-graphics
    ...
    maplace_maps: function () {
        if ($('#map_users').length) {
            ...
            var map_styles = {
                ...
                marker_url = isHighDensity() ? 'core/assets/img/md-images/ic_location_history_black_48dp.png' : 'core/assets/img/md-images/ic_location_history_black_24dp.png',
        ...
        if ($clndr_events.length) {
            ...
            $('#clndr_new_event_submit').on('click', function() {
                ...
                if($(e_title).val() == '') {
                    ...
                    uk_md.update_input($(e_title));
                    return false;
                }

                if($(e_date).val() == '') {
                    $(e_date).addClass('md-input-danger').focus();
                    uk_md.update_input($(e_date));
                    return false;
                }
                ...
                uk_md.update_input($(e_title+','+e_link+','+e_date+','+e_start+','+e_end));
