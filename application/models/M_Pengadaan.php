...
class M_Pengadaan extends CI_Model {
	...
	public function tampil_detail_pengadaan($id_det_pengadaan)
	{
		...
		$this->db->select('*')
				->from('det_pengadaan')
				->join('asset','asset.id_asset=det_pengadaan.id_asset')
	...
	{
		$this->db->select(["id_det_pengadaan", "id_pengadaan","nama_asset"])
			->from('det_pengadaan');
		$query=$this->db->get_compiled_select();
	...
	public function tampil_pengadaan_pil()
	{
		$this->db->select('*')
			->from('pengadaan')
			->where("is_inventarisasi='Belum'");
		$query=$this->db->get_compiled_select();
	...
	public function ambil_data_pengadaan($id_pengadaan)
	{
		# code...
		$this->db->select('*')
    		->from('pengadaan')
	...
	public function ambil_detail_pengadaan($id_pengadaan)
	{
		# code...
		$this->db->select('*, det_pengadaan.jumlah as jumlah_pengadaan')
    		->from('det_pengadaan')
    		->join('asset','asset.id_asset=det_pengadaan.id_asset')
    		->where("det_pengadaan.id_pengadaan", $id_pengadaan);
